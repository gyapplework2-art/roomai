...
FAILED_STATUSES = frozenset(
    {"fetch_failed", "extraction_failed", "persistence_plan_failed"}
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Review a bounded Article US sofa discovery batch."
    )
    parser.add_argument(
        "--input",
        required=True,
        type=Path,
        help="Discovery JSON produced by discover_products.",
    )
    parser.add_argument("--limit", type=int, default=DEFAULT_LIMIT)
    parser.add_argument(
        "--output",
        type=Path,
        help="Optional local JSON review output path.",
    )
...
def _review_variant(source_variant, normalized_variant) -> dict[str, object]:
    ...
    return {
        ...
        "article_attributes": source_variant.variant_attributes.get(
            "article_attributes",
            {},
        ),
        "resolved_attribute_evidence": normalized_variant.variant_attributes.get(
            "attribute_evidence",
            {},
        ),
        "source_dimension_text": (
            dimensions.source_dimension_text if dimensions else None
        ),
        "source_dimension_details": (
            dimensions.dimension_details if dimensions else {}
        ),
        "normalized_dimensions": (
            normalized_dimensions.model_dump(mode="json")
        ...
        "offer": {
            ...
            "vendor_list_price": (
                offer.vendor_list_price if offer else None
            ),
            "vendor_sale_price": (
                offer.vendor_sale_price if offer else None
            ),
            "vendor_shipping_fee": (
                offer.vendor_shipping_fee if offer else None
            ),
            "source_availability": (
                offer.source_availability if offer else None
            ),
            "normalized_availability": (
                normalized_offer.normalized_availability
                if normalized_offer
                else None
            ),
...
def _review_plan(plan: CatalogPersistencePlan) -> dict[str, object]:
    return {
        ...
        "canonical_furniture_type_code": (
            plan.canonical_furniture_type_code
        ),
        "publication_status": plan.product["publication_status"],
        "needs_taxonomy_review": plan.product[
        ...
        "variants": [
            {
                ...
                "publication_status": variant.values[
                    "publication_status"
                ],
...
def _review_resolutions(
    ...
    previous: Iterable[CatalogProduct],
) -> list[dict[str, object]]:
    proposals: list[dict[str, object]] = []

    for prior_product in previous:
        proposal: ResolutionProposal = propose_resolution(
            prior_product,
            product,
        )
...
def _quality_counts(
    ...
    successful = [
        ...
        if product["status"] in COMPLETED_STATUSES
    ]
    ...
    return {
        "missing_dimensions": sum(
            variant["normalized_dimensions"] is None
            for variant in variants
        ),
        "missing_price": sum(
            variant["offer"]["vendor_sale_price"] is None
            for variant in variants
        ),
        "missing_images": sum(
            not variant["image_urls"]
            for variant in variants
        ),
        "missing_material": sum(
            variant["source_material"] is None
            for variant in variants
        ),
        "missing_color": sum(
            variant["source_color"] is None
            for variant in variants
        ),
        "unknown_availability": sum(
            variant["offer"]["normalized_availability"] is None
            for variant in variants
        ),
    }
...
def _completed_status(
    ...
    review_reasons = list(
        dict.fromkeys(
            (*normalization_reasons, *plan_reasons)
        )
    )

    return (
        "normalization_review" if review_reasons else "success",
        review_reasons,
    )
...
async def review_candidates(
    ...
    *,
    ...
    if limit < 1 or limit > MAX_LIMIT:
        raise ValueError(
            f"Batch limit must be between 1 and {MAX_LIMIT}."
        )
    ...
    for candidate in candidates[:limit]:
            ...
            processed.append(
                {
                    ...
                    "product_url": None,
                    "stage": "input",
                    "reason": "Candidate is missing product_url.",
                }
            )
            continue
        ...
        if (
            not fetch_result.succeeded
            or fetch_result.response_text is None
        ):
            reason = (
        ...
        # Preserve explicit discovery taxonomy evidence when the product page
        # itself does not provide a source category. Extracted product-page
        # evidence always takes precedence.
        candidate_category = candidate.get("source_category")

        if (
            source_product.source_category is None
            and isinstance(candidate_category, str)
            and candidate_category.strip()
        ):
            source_product = source_product.model_copy(
                update={
                    "source_category": candidate_category.strip()
                }
            )
        ...
        try:
            ...
            processed.append(
                {
                    "status": "persistence_plan_failed",
                    "product_url": product_url,
        ...
        processed.append(
            {
                ...
                "article_page_id": source_product.source_payload.get(
                    "article_page_id"
                ),
                "vendor": source_product.source_payload.get("vendor"),
                "vendor_market_code": (
                    source_product.vendor_market_code
                ),
                "source_category": (
                    source_product.source_category
                    or candidate.get("source_category")
                ),
                "vendor_product_id": (
                    source_product.vendor_product_id
                ),
                "product_name": (
                    source_product.source_product_name
                ),
                "source_description": (
                    source_product.source_description
                ),
                "brand": source_product.source_payload.get("brand"),
                "related_products": (
                    source_product.source_payload.get(
                        "related_products"
                    )
                ),
                "attribute_evidence": (
                ...
                "variants": [
                    ...
                    for source_variant, normalized_variant in zip(
                        source_product.variants,
                        normalization.product.variants,
                        strict=True,
                    )
                ],
                "resolution_proposals": resolutions,
                "normalization_review_reasons": list(
                    normalization.review_reasons
                ),
                "persistence_review_reasons": list(
                    plan.review_reasons
                ),
    ...
    successful = sum(
        product["status"] in COMPLETED_STATUSES
        for product in processed
    )

    reviews = sum(
        product["status"] == "normalization_review"
        for product in processed
    )

    failed = sum(
        product["status"] in FAILED_STATUSES
        for product in processed
    )
...
async def run(args: argparse.Namespace) -> dict[str, object]:
    document = await review_candidates(
